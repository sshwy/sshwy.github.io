...
一个宽1或2的矩形选出k个子矩形，问最大权值和

# 分析

既然宽度只有2,状压一下每行状态DP即可

定义$f[i,j,k]$表示前$i$行选$j$个子矩形的最大权值.其中k:

- k=0表示第$i$行没有选
- k=1表示第$i$行选左边的矩形
- k=2表示第$i$行选右边的矩形
- k=3表示第$i$行选宽度为2的矩形
- k=4表示第$i$行选两个并排宽度为1的矩形
...
int solve2(){
	...
	for(int i=1;i<=n+1;i++){
		for(int j=0;j<=k;j++){
			...
			if(j>0)f[i][j][4]=max(f[i][j][4],f[i-1][j-1][1]);//左边联通，右边多一个
			if(j>0)f[i][j][4]=max(f[i][j][4],f[i-1][j-1][2]);//右边联通，左边多一个
			f[i][j][4]+=a[i][0]+a[i][1];
		}
	}
	return f[n+1][k][0];
